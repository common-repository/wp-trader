...
/*
  ...
  * @subpackage Template
*/
if( ! function_exists("torrent_upload_template") ) {
...
<?php
			...
			if(isset($_POST["takeupload"]) == "yes") {
				...
				//If file not a .torrent file then don't upload and die
				if (!isset($_FILES["torrent"])){
					$errorheader = "Upload Failed";
					$errormessage = "No .torrent file was selected.";
					die(unlink_files());
				}
				//End if file not a .torrent file then don't upload and die
			
				$f = $_FILES["torrent"];
				$fname = $f["name"];
				//If empty file name for .torrent file then don't upload and die
				if (empty($fname)){
					$errorheader = "Upload Failed";
				...
				//End if empty file name for .torrent file then don't upload and die
				
				//Check if there is an nfo file and that the size is not zero
				if ($_FILES['nfo']['size'] != 0) {
					$nfofile = $_FILES['nfo'];
				//End check if there is an nfo file and that the size is not zero
				
					//Check if the nfo file name is not empty if so then don't upload and die
					if ($nfofile['name'] == ''){
						$errorheader = "Upload Failed";
						$errormessage = "No NFO!";
						wptrader_update($errorheader, $errormessage);
					...
					//End Check if the nfo file name is not empty if so then don't upload and die
					
					//Check if the file for the .nfo extension if it does not have it then don't upload and die
					...
					//End check if the file for the .nfo extension if it does not have it then don't upload and die
					...
					//Check if there is an nfo file and that the size is not to big per the options
					...
					//End check if there is an nfo file and that the size is not to big per the options
				...
				//Check if there is a description if there is not desription the error out
				$descr = $_POST["descr"];
				if (!$descr){
					$errorheader = "Upload Failed";
				...
				//End check if there is a description if there is not desription the error out
				
				$langid = (0 + $_POST["lang"]);
		
				$catid = (0 + $_POST["type"]);
				
				//Check if valid category id if not then error out
				...
				//End check if valid category id if not then error out
				
				//Check if valid filename if not error out
				...
				//End check if valid filename if not error out
				
				//Check if valid .torrent file extension if not error out
				...
				//End check if valid .torrent file extension if not error out
			
				$shortfname = $torrent = $matches[1];
				
				//Check if empty torrent name if it is then it will be taken from the .torrent filename
				if (!empty($_POST["name"]))
					$torrent = $_POST["name"];
				//End check if empty torrent name if it is then it will be taken from the .torrent filename
				...
				//End check announce url is local or external
				
				//Check if externals is turned off
				...
				//End check if externals is turned off
				...
				//Check if torrent is uploaded or not already if it is then error out
				...
				//End check if torrent is uploaded or not already if it is then error out
				
				//Check if torrent browse page is used or not. If not used then publish like normal post
				...
				//End check if torrent browse page is used or not.
