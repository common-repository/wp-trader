...
if( ! function_exists("wptrader_install") ) {
	function wp_trader_install_options() {
		...
		$default_latest_uploaded_name_shortner = '10';
		...
		$default_seed_wanted_name_shortner = '10';
...
if( ! function_exists("wp_trader_install_pages") ) {
	function wp_trader_install_pages() {
		...
		if (is_writable(get_stylesheet_directory())) {
			$source_announce = WP_TRADER_ABSPATH . '/templates/announce.php';
			$source_scrape = WP_TRADER_ABSPATH . '/templates/scrape.php';
			$destination_announce = get_stylesheet_directory() . '/announce.php';
			$destination_scrape = get_stylesheet_directory() . '/scrape.php';
			if (!file_exists($destination_announce) && file_exists($source_announce)) {
				$data_announce = file_get_contents($source_announce);
				$handle_announce = fopen($destination_announce, 'w');
				fwrite($handle_announce, $data_announce);
				fclose($handle_announce);
			}
			if (!file_exists($destination_scrape) && file_exists($source_scrape)) {
				$data_scrape = file_get_contents($source_scrape);
